...
export const ErrorModal = ({ error, errorFront, seterrorModal, cerrarModal }) => {

  const closeModal = () => {
    seterrorModal(false)
    cerrarModal(false)
  }
  const errorMessage = (error) ? error : errorFront
  ...
  return (
    <>
      <div className="w-[500px] h-[200px] z-10 bg-white absolute m-auto top-[50%] left-[50%] translate-x-[-250px] translate-y-[-100px] rounded-[40px] animate__zoomIn animate__animated animate__faster 200ms flex flex-col z-10">
        ...
        <p className='text-center text-[20px] mt-[80px] self-center absolute'>{errorMessage}</p>
      </div>
    </>
  )
}
