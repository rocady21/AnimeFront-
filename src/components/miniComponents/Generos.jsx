  ...
  return (
    ...
    <div className='relative inline-block w-[120px]'>
      ...
      </div>
      {
        state === true && (
          <div
          className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md h-[200px] overflow-auto bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
          role="menu"
              ...
              generos?.map((generounique) => {
                return (
                  <a
                    ...
                    id="menu-item-0"
                  >
                    <CheckGenero genero={generounique} key={generounique}  guardar={(genero)=>guardarValoresSeleccionados(genero)} quitar={(genero)=> borrarValoresSeleccionados(genero)}/>
                  </a>
                );
              })
            ) : (
    ...
    <div className='listadoGenerosSeleccionados max-h-[100px] mt-[20px] max-w-full  flex flex-row flex-wrap'>
      {
        ...
        (
          generosSelected.map((generocheck)=> {
            return <div key={generocheck} className='px-[10px] py-[4px] mx-[3px] my-[10px] bg-gray-300 rounded-full '>{generocheck} <button onClick={()=> borrarValoresSeleccionados(generocheck)}>X</button></div>
          })
          ):
